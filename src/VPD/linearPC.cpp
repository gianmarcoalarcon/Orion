										 ...
										 long long input_depth)
{
	...
	for (int i = 0; i < C[recursion_depth].R; ++i)
	{
		...
		v.C.circuit[input_depth + 1].gates[output_size_so_far + i].src = &v.C.circuit[input_depth +
																																									1]
																																					.src_expander_C_mempool[mempool_ptr];
	...
	for (long long i = 0; i < D[recursion_depth].R; ++i)
	{
		...
		v.C.circuit[final_output_depth].gates[output_size_so_far +
																					i]
				.src = &v.C.circuit[final_output_depth].src_expander_D_mempool[mempool_ptr];
		v.C.circuit[final_output_depth].gates[output_size_so_far +
																					i]
				.weight = &v.C.circuit[final_output_depth].weight_expander_D_mempool[mempool_ptr];
...
void generate_circuit(long long *query, long long N, int query_count, prime_field::field_element *input)
{
	...
	for (long long i = 0; i < D[0].R; ++i)
	{
		...
		v.C.circuit[final_output_depth].gates[output_so_far +
																					i]
				.src = &v.C.circuit[final_output_depth].src_expander_D_mempool[D_mempool_ptr];
		v.C.circuit[final_output_depth].gates[output_so_far +
																					i]
				.weight = &v.C.circuit[final_output_depth].weight_expander_D_mempool[D_mempool_ptr];
