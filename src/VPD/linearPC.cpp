...
__hhash_digest *commit(const prime_field::field_element *src, long long N) {
    ...
    for (int i = 0; i < column_size; ++i) {
        ...
        printf("ENCODED %d %llu %llu\n", i, encoded_codeword[i][128].real, encoded_codeword[i][128].img);
    }

    printf("FINISH %d %llu %llu\n", 0, encoded_codeword[0][717].real, encoded_codeword[0][717].img);

    for (int i = 0; i < N / column_size * 2; ++i) {
        memset(&stash[i], 0, sizeof(__hhash_digest));
        for (int j = 0; j < column_size / 2; ++j) {
            stash[i] = merkle_tree::hash_double_field_element_merkle_damgard(encoded_codeword[2 * j][i],
                                                                             encoded_codeword[2 * j + 1][i], stash[i]);
        }
    }
...
generate_enc_circuit(long long input_size, long long output_size_so_far, long long recursion_depth,
                     long long input_depth) {
    if (input_size <= distance_threshold) {
...
//        }
        ...
        v.C.circuit[input_depth + 1].gates[output_size_so_far + i].src = &v.C.circuit[input_depth +
                                                                                      1].src_expander_C_mempool[mempool_ptr];
        v.C.circuit[input_depth + 1].gates[output_size_so_far + i].weight = &v.C.circuit[input_depth +
                                                                                         1].weight_expander_C_mempool[mempool_ptr];
    ...
    auto output_depth_output_size = generate_enc_circuit(C[recursion_depth].R,
                                                         output_size_so_far + C[recursion_depth].R, recursion_depth + 1,
                                                         input_depth + 1);
    ...
    v.C.circuit[final_output_depth].weight_expander_D_mempool = new prime_field::field_element[dn *
                                                                                               D[recursion_depth].L];
...
//        }
        ...
        v.C.circuit[final_output_depth].gates[output_size_so_far +
                                              i].src = &v.C.circuit[final_output_depth].src_expander_D_mempool[mempool_ptr];
        v.C.circuit[final_output_depth].gates[output_size_so_far +
                                              i].weight = &v.C.circuit[final_output_depth].weight_expander_D_mempool[mempool_ptr];
...
//        }
        ...
        v.C.circuit[final_output_depth].gates[output_so_far +
                                              i].src = &v.C.circuit[final_output_depth].src_expander_D_mempool[D_mempool_ptr];
        v.C.circuit[final_output_depth].gates[output_so_far +
                                              i].weight = &v.C.circuit[final_output_depth].weight_expander_D_mempool[D_mempool_ptr];
...
tensor_product_protocol(prime_field::field_element *r0, prime_field::field_element *r1, long long size_r0,
                        long long size_r1, long long N, __hhash_digest *com_mt) {
        ...
        for (int j = 0; j < codeword_size[0]; ++j) {
            ...
            if (i == 0 && j <= 50) {
                printf(
                        ...
                        combined_codeword[j].img, r0[i].real, r0[i].img, encoded_codeword[i][j].real,
                        encoded_codeword[i][j].img
                       );
            }
        }
    }
    ...
    merkle_tree::merkle_tree_prover::create_tree(combined_codeword_hash, N / column_size * 2, combined_codeword_mt,
    ...
    for (int i = 0; i < query_count; ++i) {
        ...
        for (int j = 0; j < column_size / 2; ++j) {
            column_hash = merkle_tree::hash_double_field_element_merkle_damgard(encoded_codeword[2 * j][q],
                                                                                encoded_codeword[2 * j + 1][q],
                                                                                column_hash);
        }

        assert(merkle_tree::merkle_tree_verifier::verify_claim(com_mt[1], com_mt, column_hash, q, N / column_size * 2,
                                                               visited_com, proof_size));
        assert(merkle_tree::merkle_tree_verifier::verify_claim(combined_codeword_mt[1], combined_codeword_mt,
                                                               merkle_tree::hash_single_field_element(
                                                                       combined_codeword[q]), q, N / column_size * 2,
                                                               visited_combined_com, proof_size));
...
//        if (i == 718 || i == 719) {
//            printf("ASSERT %llu %llu %lld\n", p.circuit_value[p.C->total_depth - 1][i].real,
//                   p.circuit_value[p.C->total_depth - 1][i].img, q[i]);
//            printf("ASSERT %llu %llu %lld\n", combined_codeword[q[i]].real, combined_codeword[q[i]].img, q[i]);
//        }
    }
...
std::pair<prime_field::field_element, bool>
open_and_verify(prime_field::field_element x, long long N, __hhash_digest *com_mt) {
    assert(N % column_size == 0);
...
void dfs(prime_field::field_element *dst, prime_field::field_element *r, int size, int depth,
         prime_field::field_element val) {
    if (size == 1) {
        *dst = val;
    } else {
...
std::pair<prime_field::field_element, bool>
open_and_verify(prime_field::field_element *r, int size_r, int N, __hhash_digest *com_mt) {
    assert(N % column_size == 0);
    prime_field::field_element *r0, *r1;
