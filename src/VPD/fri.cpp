...
#pragma omp parallel for
		for (int j = 0; j < slice_number; ++j)
		{
			...
			// printf("i:%d, j:%d, inv_2.real:%lld, img:%lld\n", i, j, inv_2.real, inv_2.img);
			// printf("previous_witness[%d].real:%lld, img:%lld\n", real_pos, previous_witness[real_pos].real, previous_witness[real_pos].img);
			// printf("previous_witness[%d].real:%lld, img:%lld\n", real_pos | 1, previous_witness[real_pos].real, previous_witness[real_pos].img);
			// printf("inv_mu.real:%lld, img:%lld\n", inv_mu.real, inv_mu.img);
			// cpd.rs_codeword[current_step_no][i << log_slice_number | j] =
			// 		inv_2 * ((previous_witness[real_pos] + previous_witness[real_pos | 1]) + inv_mu * r * (previous_witness[real_pos] - previous_witness[real_pos | 1]));
			// printf("cpd.rs_codeword[%d][%d].real:%lld, img:%lld\n\n", current_step_no, i << log_slice_number | j, cpd.rs_codeword[current_step_no][i << log_slice_number | j].real, cpd.rs_codeword[current_step_no][i << log_slice_number | j].img);
		}
		// if (i > 8)
		// {
		// 	exit(0);
