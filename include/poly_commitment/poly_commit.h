...
#pragma omp parallel for
				for (int j = 0; j < slice_size; ++j)
				{
					...
					auto p = l_eval[i * slice_size + j];
					auto q = q_eval[i * slice_size + j];
					auto aabb = twiddle_factor[twiddle_gap * j % twiddle_factor_size] - 1;
					auto h = h_eval[j];
					// printf("p.real:%lld, img:%lld\n", p.real, p.img);
					// printf("q.real:%lld, img:%lld\n", q.real, q.img);
					// printf("aabb.real:%lld, img:%lld\n", aabb.real, aabb.img);
					printf("h.real:%lld, img:%lld\n", h.real, h.img);
					auto g = p * q - aabb * h;
					printf("g.real:%lld, img:%lld\n", g.real, g.img);
					if (j < slice_size / 2)
					{
						...
						// printf("1 j:%d, i:%d, \n", j, i);
						// printf("fri::virtual_oracle_witness[%d].real:%lld, img:%lld\n\n", (j) << log_leaf_size | (i << 1) | 0, fri::virtual_oracle_witness[(j) << log_leaf_size | (i << 1) | 0].real, fri::virtual_oracle_witness[(j) << log_leaf_size | (i << 1) | 0].img);
					}
					else
					{
						...
						// printf("2 j:%d, i:%d, \n", j, i);
						// printf("fri::virtal_oracle_witness[%d].real:%lld, img:%lld\n\n", (jj) << log_leaf_size | (i << 1) | 1, fri::virtual_oracle_witness[(jj) << log_leaf_size | (i << 1) | 1].real, fri::virtual_oracle_witness[(jj) << log_leaf_size | (i << 1) | 1].img);
					}
					/*
